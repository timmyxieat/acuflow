...
- **Auto-hide scrollbar**: Default behavior - scrollbar appears on scroll, fades after 1.5s
- **Hidden scrollbar**: Use `hideScrollbar` prop for touch-first areas
...
PatientCards supports two display modes:
...
### Route-Aware Header (HeaderContext)
...
useEffect(() => {
  ...
  return () => resetHeader() // Reset when leaving page
...
- When `setHeader` is called, the context stores the current page's `title` (if it has one)
- Detail pages can access `previousTitle` to show contextual back button text (e.g., "Back to Today")
- This is tracked via a ref to avoid unnecessary re-renders

**Important:** The dependency array should only include the page/item ID, not `setHeader`/`resetHeader` functions, to avoid infinite re-render loops.

### Page Transitions (TransitionContext)

Use `TransitionContext` to coordinate animations across route changes:
...
// For vertical slides between items, set direction before navigating
...
### Keyboard Navigation & Hover State

Use `useHoverWithKeyboardNav` hook for consistent hover behavior that respects keyboard navigation:

```typescript
import { useHoverWithKeyboardNav } from '@/hooks/useHoverWithKeyboardNav'

// Returns [rawHoveredId, setHoveredId, effectiveHoveredId]
const [, setHoveredId, effectiveHoveredId] = useHoverWithKeyboardNav<string>()

// Pass effectiveHoveredId to components - it's null during keyboard nav mode
<PatientCards
  onAppointmentHover={setHoveredId}
  hoveredAppointmentId={effectiveHoveredId}
/>
```

**How it works:**
- When arrow keys are used, `setKeyboardNavMode(true)` is called (via TransitionContext)
- `effectiveHoveredId` returns `null` during keyboard nav mode, suppressing hover highlights
- When mouse moves, keyboard nav mode ends and hover becomes active again

**Keyboard shortcuts (TodayScreen):**
- `ESC`: Deselect card (stay on Today)
- `Enter`: Navigate to selected card, or re-select last card if none selected
- `↑/↓ Arrow`: Move selection between patient cards

**Keyboard shortcuts (Appointment Detail):**
- `ESC`: Deselect visit (if selected), or return to Today
- `↑/↓ Arrow`: Navigate visits (if visit selected), or navigate patient cards
- Arrow keys are disabled when focused on input/textarea
...
import {
  ...
  CONTENT_SLIDE_ANIMATION, // Horizontal (Timeline) and vertical (between appointments) slides
...
**Update once, synchronized everywhere.** All animation configs in one place.

### Layout Animations (Framer Motion)

For smooth collapse/expand animations, use Framer Motion's layout features:

```typescript
import { motion, AnimatePresence, LayoutGroup } from 'framer-motion'

// Wrap related elements in LayoutGroup
<LayoutGroup>
  {items.map(item => (
    <motion.div
...
**Key patterns:**
- `layoutId` for elements that morph position between states
- `AnimatePresence` for elements that mount/unmount
- `layout="position"` on buttons/cards (animates position only, not size - avoids height jumping)
- Avoid `layout` on containers that don't need size animation
- Use shared `layoutId` for selection indicators that slide between items
- Spring transitions for snappy, natural feel

### Appointment Type Icons

Use consistent icon mapping for appointment types (defined in `Timeline.tsx` and `appointments/[id]/page.tsx`):
...
// Usage - simple muted style without background
...
The `icon` field on `AppointmentType` model stores Lucide icon names, but currently we use a hardcoded mapping by ID for consistency.
