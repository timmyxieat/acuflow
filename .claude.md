...
- Use symmetric padding (`px-3` = 12px) for scrollable areas
- Use `h-dvh` instead of `h-screen` for viewport height (iPad browser compatibility)
...
### Responsive Widths (Hydration-Safe)

Use CSS for responsive widths instead of JS calculations to avoid hydration mismatches:

```tsx
// Bad: JS-calculated width causes hydration mismatch
const [width, setWidth] = useState(getWidth()) // Different on server vs client
<motion.div animate={{ width }}>

// Good: CSS handles responsive width, JS only for collapsed state
const PANEL_WIDTH_CLASS = 'w-[clamp(180px,20vw,280px)]'
<div
  className={`transition-[width] ${isCollapsed ? '' : PANEL_WIDTH_CLASS}`}
  style={{ width: isCollapsed ? 64 : undefined }}
>
```

### Skipping Animation on Initial Mount

When URL params control state that affects animation, skip animation on first render:
...
// CSS transition only after initialization
...
Keep files under ~700 lines to stay within Claude's ~25,000 token read limit. For complex pages, split into co-located modules:

```
src/app/[route]/
├── page.tsx           # Main component (~400-700 lines)
├── components/        # Page-specific UI components
├── hooks/             # Page-specific hooks
└── lib/               # Constants, helpers, types
```
